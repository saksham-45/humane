...
export function stampForPick(picked: Side, humanSide: Side): "HUMAN" | "AI" {
...
let stampClip: HTMLAudioElement | null = null;
...
export async function unlockStampAudio(): Promise<void> {
...
function synthStampHit(ctx: AudioContext): void {
...
export function playStampHit(): void {
  if (typeof window === "undefined") return;
...
export type SweepHandles = {
  ...
  leftStamp: HTMLElement;
  rightStamp: HTMLElement;
};

function faceOf(stamp: HTMLElement): HTMLElement {
  return (stamp.querySelector(".ink-stamp-face") as HTMLElement | null) ?? stamp;
}
...
function slamStamp(stamp: HTMLElement, kind: "HUMAN" | "AI"): void {
  stamp.hidden = false;
  stamp.classList.remove("is-in", "is-human", "is-ai");
...
export class Sweep {
  constructor(private readonly handles: SweepHandles) {}
